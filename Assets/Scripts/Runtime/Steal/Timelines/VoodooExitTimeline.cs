...
{
    /// <summary>
    /// Exit timeline for the voodoo-steal mini-game. Today it only publishes
    /// the session-ended signal — that's enough for the existing presenters
    /// (doll hides, victim-name label clears). The class exists as the
    /// future home of the exit cinematic: doll-shatter animation, exit
    /// sound, camera-return-to-city kick-off, etc. Future code goes inside
    /// RunAsync; the coordinator's awaiting site doesn't change.
    ///
    ...
    /// already false).
    /// </summary>
    public sealed class VoodooExitTimeline
    {
        private readonly IPublisher<VoodooSessionEndedSignal> sessionEndedPublisher;

        [Inject]
        public VoodooExitTimeline(IPublisher<VoodooSessionEndedSignal> sessionEndedPublisher)
        {
